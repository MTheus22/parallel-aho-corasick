...
> **⚠️ ATUALIZAÇÃO (epic-03, 2026-06-29).** Este README descreve a **1ª corrida
> REDUZIDA** (2026-06-25, grade `A B D E`, 4 searchers, sem topology-aware) via o
> `run_workstation_sweep.sh` — hoje **legado**. O epic-03 inverte os papéis: a
> workstation passa a ser o **ambiente canônico** e roda a **grade COMPLETA A–E**
> (9 paralelos + 2 seq, incl. topology-aware) pelo **motor unificado**. Para a
> nova corrida, use:
> ```bash
...
>   ./scripts/run_all.sh
> ```
> (`run_all.sh` → `prepare_data.sh` + governador + build/test + `run_sweep.sh`
> desacoplado + upload.) A descrição de grade/searchers abaixo reflete a corrida
> reduzida antiga; a grade completa em T=32 está em `docs/sweep-test-inventory.md`.
...
```bash
./scripts/prepare_data.sh                    # só siga se o veredito for "PRONTO"
...
**Forma canônica (epic-03) — um comando** faz tudo (dados → governador → build/
test → sweep desacoplado → upload):

```bash
RUN_DIR=runs/workstation ./scripts/run_all.sh     # + AC_GIT_PULL/AC_GIT_PUSH/AC_NOTIFY opcionais
```

Forma manual equivalente (se quiser cada passo à mão):

```bash
./scripts/prepare_data.sh                          # 1. dados → "PRONTO"
sudo cpupower frequency-set -g performance         # 2. governor performance
make && make test                                  # 3. correção no chip-alvo
RUN_DIR=runs/workstation nohup ./scripts/run_sweep.sh > workstation.out 2>&1 &   # 4. sweep completo A–E
```

- **A grade canônica do epic-03 é a COMPLETA `A B C D E`** (default do
  `run_sweep.sh`), não a reduzida `A B D E` da 1ª corrida. A fase `G`
  (granularidade) segue opt-in (`PHASES="G"`).
- (Legado) o antigo `run_workstation_sweep.sh` rodava `A B D E` com `S` opcional
  via `PHASES="A B D E S"`.
...
|---|---|---|
| Muitos `.FAIL` em A/D | `enron_x8.txt` ausente | rode `prepare_data.sh` |
...
- Motor do sweep (unificado): `scripts/run_sweep.sh` (legado: `run_workstation_sweep.sh`)
- Wrapper "um comando": `scripts/run_all.sh` (legado: `workstation_all.sh`)
- Pré-flight de dados: `scripts/prepare_data.sh` (legado: `prepare_workstation_data.sh`)
